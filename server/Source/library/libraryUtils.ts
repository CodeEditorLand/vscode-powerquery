...
export const SdkLibrarySymbols: ReadonlyMap<
	string,
	ReadonlyArray<PQLS.LibrarySymbol.LibrarySymbol>
...
export function createLibrary(
	staticLibraryDefinitionCollection: ReadonlyArray<
		ReadonlyArray<PQLS.LibrarySymbol.LibrarySymbol>
	>,
	dynamicLibraryDefinitionCollection: ReadonlyArray<
		() => ReadonlyMap<string, PQLS.Library.TLibraryDefinition>
	>,
): PQLS.Library.ILibrary {
	const staticLibraryDefinitions: Map<
		string,
		PQLS.Library.TLibraryDefinition
	> = new Map();

	for (const collection of staticLibraryDefinitionCollection) {
		for (const [key, value] of LibrarySymbolUtils.toLibraryDefinitions(
			collection,
		).entries()) {
	...
	const dynamicLibraryDefinitions: () => ReadonlyMap<
		string,
		PQLS.Library.TLibraryDefinition
	> = () => {
...
export function getLibrary(
	cacheKey: string,
): PQLS.Library.ILibrary | undefined {
