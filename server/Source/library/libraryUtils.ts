...
import {
	createLibraryTypeResolver,
	LibraryDefinitionsGetter,
} from "./libraryTypeResolver";
import {
	PartialResult,
	PartialResultUtils,
...
function getOrCreateLibrary(
	...
	const libraryOfNoExternals: Library.ILibrary = PQP.Assert.asDefined(
		libraryByLocale.get(locale)
	);
...
const sdkLibraryDefinitionsByLocale: Map<string, Library.LibraryDefinitions> =
	new Map();

const sdkLibrarySymbols: Map<
	string,
	ReadonlyArray<LibrarySymbol.LibrarySymbol>
> = new Map([[PQP.Locale.en_US, SdkLibrarySymbolsEnUs]]);

const standardLibraryByLocale: Map<string, Library.ILibrary> = new Map();
const standardLibraryDefinitionsByLocale: Map<
	string,
	Library.LibraryDefinitions
> = new Map();

const standardLibrarySymbolByLocale: Map<
	string,
	ReadonlyArray<LibrarySymbol.LibrarySymbol>
