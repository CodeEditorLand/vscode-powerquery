...
export function createLibraryTypeResolver(
	...
	return (
		...
		let maybeLibraryType: Type.TPowerQueryType | undefined =
			libraryDefinitions.get(request.identifierLiteral)?.asPowerQueryType;

		if (!maybeLibraryType) {
			otherLibraryDefinitionsGetters.some(
				(oneLibraryDefinitionGetter: LibraryDefinitionsGetter) => {
					const oneLibraryDefinitions: PQLS.Library.LibraryDefinitions =
						oneLibraryDefinitionGetter();
					maybeLibraryType = oneLibraryDefinitions.get(
		...
		// It's asking for a value, which we already have.
			...
			const typeChecked: TypeUtils.CheckedInvocation =
				TypeUtils.typeCheckInvocation(
					request.args,
...
function isValidInvocation(typeChecked: TypeUtils.CheckedInvocation): boolean {
	return (
		!typeChecked.extraneous.length &&
		!typeChecked.invalid.size &&
		!typeChecked.missing.length
	);
}
...
function resolveTableAddColumn(
	...
	const maybeColumnType: Type.TPowerQueryType | undefined =
		args.length === 4
			? TypeUtils.assertAsType(PQP.Assert.asDefined(args[3]))
			: undefined;
	...
	const normalizedColumnName: string =
		PQP.Language.TextUtils.normalizeIdentifier(
