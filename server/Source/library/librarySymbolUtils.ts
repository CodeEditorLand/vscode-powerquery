...
export function getSymbolsForLocaleAndMode(
	...
	switch (mode) {
		case "Power Query":
			return (
				StandardLibrarySymbolByLocale.get(locale) ??
...
export function toLibraryDefinitions(
	...
	let libraryDefinitions: ReadonlyMap<
		string,
	...
	if (invalidSymbols.length) {
		console.warn(
			`Failed to convert library symbols: ${JSON.stringify(invalidSymbols)}`,
		);
	}
...
const StandardLibrarySymbolByLocale: ReadonlyMap<
	string,
	ReadonlyArray<PQLS.LibrarySymbol.LibrarySymbol>
> = new Map([[PQP.Locale.en_US, StandardLibrarySymbolsEnUs]]);

const SdkLibrarySymbols: ReadonlyMap<
	string,
	ReadonlyArray<PQLS.LibrarySymbol.LibrarySymbol>
