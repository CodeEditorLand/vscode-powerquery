...
export class ModuleLibraryTreeNode {
	...
	private _libraryDefinitions: ReadonlyMap<
		string,
	...
	set libraryJson(
		val: ReadonlyArray<LibrarySymbol.LibrarySymbol> | undefined
	) {
		...
		if (val && Array.isArray(val)) {
			...
			if (PartialResultUtils.isOk(libraryDefinitionsResult)) {
				...
				PartialResultUtils.isIncomplete(libraryDefinitionsResult)
			) {
				libraryDefinitions =
			...
			if (failedSymbols.length) {
				const csvSymbolNames: string = failedSymbols
					.map(
						(librarySymbol: LibrarySymbol.LibrarySymbol) =>
							librarySymbol.name
					)
					.join(", ");
	...
	public libraryDefinitionsGetter: LibraryDefinitionsGetter = () =>
		this.libraryDefinitions;

	public readonly children: Map<string, ModuleLibraryTreeNode> = new Map();

	constructor(
		private readonly parent?: ModuleLibraryTreeNode,
	...
	insert(
		paths: string[],
		visitorContext?: {
			closestModuleLibraryTreeNodeOfDefinitions: ModuleLibraryTreeNode;
		}
		...
		const maybeOneChild: ModuleLibraryTreeNode | undefined =
			this.children.get(currentPath);

		if (maybeOneChild) {
			child = maybeOneChild;

			if (visitorContext && child.libraryJson) {
				visitorContext.closestModuleLibraryTreeNodeOfDefinitions =
					child;
			}
		} else {
	...
	collectTextDocumentBeneath(visitorContext: {
...
export class ModuleLibraries {
	private readonly trieRoot: ModuleLibraryTreeNode =
		ModuleLibraryTreeNode.defaultRoot;
	private readonly openedTextDocumentTreeNodeMap: Map<
		string,
		ModuleLibraryTreeNode
	> = new Map();
	...
	addOneModuleLibrary(
		uriPath: string,
		libraryJson: ReadonlyArray<LibrarySymbol.LibrarySymbol>
	): TextDocument[] {
	...
	addOneTextDocument(textDocument: TextDocument): ModuleLibraryTreeNode {
		const spitedPath: string[] = ModuleLibraries.splitPath(
			textDocument.uri
		);
		...
		const theNode: ModuleLibraryTreeNode = this.trieRoot.insert(
			spitedPath,
			visitorContext
		);
