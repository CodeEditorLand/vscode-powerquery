...
type AbridgedFormatErrorMetadata = Pick<
	FormatErrorMetadata,
	"message" | "name"
> & {
...
function stripTopOfStack(
	obj: FormatErrorMetadata
...
describe(`errorUtils`, () => {
	describe(`formatError`, () => {
		...
		it(`InvariantError`, () => {
			const actual: AbridgedFormatErrorMetadata = abridgedFormattedError(
				formatError(
					new PQP.CommonError.CommonError(
						new PQP.CommonError.InvariantError("1 <> 2")
					)
				)
			);
