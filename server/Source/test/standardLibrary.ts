...
const NoOpCancellationTokenInstance: NoOpCancellationToken =
	new NoOpCancellationToken();

async function assertGetHover(text: string): Promise<Hover> {
	const [analysis, position]: [PQLS.Analysis, Position] =
		createAnalysis(text);

	const result: PQP.Result<
		PQLS.Hover | undefined,
		PQP.CommonError.CommonError
	> = await analysis.getHover(position, NoOpCancellationTokenInstance);
...
async function assertGetSignatureHelp(text: string): Promise<SignatureHelp> {
	...
	const result: PQP.Result<
		...
		PQP.CommonError.CommonError
	> = await analysis.getSignatureHelp(
		position,
...
) => asserts definition is PQLS.Library.LibraryFunction =
	PQLS.LibraryUtils.assertIsFunction;
...
function assertIsMarkupContent(
	value: Hover["contents"]
): asserts value is MarkupContent {
...
function createAnalysis(textWithPipe: string): [PQLS.Analysis, Position] {
	...
	const library: PQLS.Library.ILibrary =
		LibraryUtils.getOrCreateStandardLibrary();

	const analysisSettings: AnalysisSettings = {
		inspectionSettings: PQLS.InspectionUtils.inspectionSettings(
			PQP.DefaultSettings,
			{ library }
		),
	...
	return [
		PQLS.AnalysisUtils.analysis(
			PQLS.textDocument(textWithPipe, 1, text),
			analysisSettings
		),
		position,
	];
	...
	describe(`simple`, () => {
		it("index const by name", () => {
			...
			const libraryDefinition:
				...
				| undefined = library.libraryDefinitions.get(definitionKey);

			if (libraryDefinition === undefined) {
				throw new Error(
					`expected constant '${definitionKey}' was not found`
				);
			}

			expect(libraryDefinition.label).eq(
				definitionKey,
				"unexpected label"
			);
			expect(libraryDefinition.description.length).greaterThan(
				0,
				"summary should not be empty"
			);
			expect(libraryDefinition.kind).eq(
				PQLS.Library.LibraryDefinitionKind.Constant
			);
			expect(libraryDefinition.asPowerQueryType.kind).eq(
		...
		it("index function by name", () => {
			...
			const libraryDefinition:
				| PQLS.Library.TLibraryDefinition
				| undefined = library.libraryDefinitions.get(exportKey);

			if (libraryDefinition === undefined) {
				throw new Error(
					`expected constant '${exportKey}' was not found`
				);
			}
			...
			expect(libraryDefinition.parameters[0].typeKind).eq(
				PQP.Language.Type.TypeKind.List
			);
			expect(libraryDefinition.parameters[1].typeKind).eq(
				PQP.Language.Type.TypeKind.Any
			);
		});
		...
		it("#date constructor", () => {
			...
			expect(libraryDefinition.label !== null);
			expect(libraryDefinition.kind).eq(
				PQLS.Library.LibraryDefinitionKind.Function
			);
			expect(libraryDefinition.parameters.length).eq(
				3,
	...
	describe(`StandardLibrary `, () => {
		describe(`Table.AddColumn`, () => {
			...
			it("getSignatureHelp", async () => {
				...
				const signature: SignatureInformation = PQP.Assert.asDefined(
				...
				const parameters: ReadonlyArray<ParameterInformation> =
					signature.parameters;

				const firstParameter: ParameterInformation =
					PQP.Assert.asDefined(parameters[0]);
				expect(firstParameter.label).to.equal(
					PQP.Language.Constant.PrimitiveTypeConstant.Table
				);

				const secondParameter: ParameterInformation =
					PQP.Assert.asDefined(parameters[1]);
				expect(secondParameter.label).to.equal("newColumnName");

				const thirdParameter: ParameterInformation =
					PQP.Assert.asDefined(parameters[2]);
				expect(thirdParameter.label).to.equal("columnGenerator");

				const fourthParameter: ParameterInformation =
					PQP.Assert.asDefined(parameters[3]);
