...
const debouncedValidateDocument: (
	this: unknown,
	textDocument: PQLS.TextDocument
) => Promise<void> = FuncUtils.partitionFn(
	() => FuncUtils.debounce(validateDocument, 250),
	(textDocument: TextDocument) =>
		`validateDocument:${textDocument.uri.toString()}`
);
...
connection.onCompletion(
	async (
		...
		const result: PQP.Result<
			PQLS.Inspection.AutocompleteItem[] | undefined,
			PQP.CommonError.CommonError
		> = await analysis.getAutocompleteItems(
			params.position,
...
documents.onDidChangeContent(
	async (event: LS.TextDocumentChangeEvent<TextDocument>) =>
		await debouncedValidateDocument(event.document)
);
...
connection.onHover(
	async (
		...
		const result: PQP.Result<
			LS.Hover | undefined,
			PQP.CommonError.CommonError
		> = await analysis.getHover(params.position, pqpCancellationToken);
...
connection.onInitialize((params: LS.InitializeParams) => {
	...
	SettingsUtils.setHasConfigurationCapability(
		Boolean(params.capabilities.workspace?.configuration)
	);
...
connection.onRenameRequest(
	async (params: RenameParams, cancellationToken: LS.CancellationToken) => {
		const document: TextDocument | undefined = documents.get(
		...
		const result: PQP.Result<
			LS.TextEdit[] | undefined,
			PQP.CommonError.CommonError
		> = await analysis.getRenameEdits(
			params.position,
			params.newName,
			pqpCancellationToken
		);
		...
		if (PQP.ResultUtils.isOk(result)) {
			return result.value
				? { changes: { [document.uri]: result.value } }
				: undefined;
		} else {
			ErrorUtils.handleError(
				connection,
...
connection.onRequest(
	...
	async (params: SemanticTokenParams) => {
		const document: TextDocument | undefined = documents.get(
		...
		const result: PQP.Result<
			PQLS.PartialSemanticToken[] | undefined,
			PQP.CommonError.CommonError
...
connection.onRequest(
	...
	(params: ModuleLibraryUpdatedParams): void => {
		const allTextDocuments: TextDocument[] =
			moduleLibraries.addOneModuleLibrary(
...
connection.onDocumentFormatting(
	async (
		params: LS.DocumentFormattingParams,
		cancellationToken: LS.CancellationToken
	): Promise<LS.TextEdit[]> => {
		const document: TextDocument | undefined = documents.get(
			params.textDocument.uri
...
const onValidateCancellationTokens: Map<string, PQP.ICancellationToken> =
	new Map();
...
function createAnalysis(
	...
	const localizedLibrary: PQLS.Library.ILibrary =
		getLocalizedModuleLibraryFromTextDocument(moduleLibraries, document);

	return PQLS.AnalysisUtils.analysis(
		document,
...
async function documentSymbols(
	...
	const triedParseState: PQP.Result<
		PQP.Parser.ParseState | undefined,
		PQP.CommonError.CommonError
...
async function validateDocument(document: TextDocument): Promise<void> {
	const traceManager: PQP.Trace.TraceManager =
		TraceManagerUtils.createTraceManager(document.uri, "validateDocument");

	const localizedLibrary: PQLS.Library.ILibrary =
		getLocalizedModuleLibraryFromTextDocument(
			moduleLibraries,
			document,
			true
		);
	...
	const validationSettings: PQLS.ValidationSettings =
		SettingsUtils.createValidationSettings(localizedLibrary, traceManager);

	const uri: string = document.uri.toString();
	const existingCancellationToken: PQP.ICancellationToken | undefined =
		onValidateCancellationTokens.get(uri);

	if (existingCancellationToken !== undefined) {
		existingCancellationToken.cancel(
			"A new validateDocument call was made."
		);
		onValidateCancellationTokens.delete(uri);
	}

	const newCancellationToken: PQP.ICancellationToken | undefined =
	...
	const result: PQP.Result<
		PQLS.ValidateOk | undefined,
		PQP.CommonError.CommonError
	> = await PQLS.validate(document, analysisSettings, validationSettings);
