...
connection.onRequest(
	...
	EventHandlerUtils.genericRequestHandler(
		(params: ModuleLibraryUpdatedParams) => {
			ModuleLibraryUtils.onModuleAdded(
...
connection.onRequest(
	...
	EventHandlerUtils.genericRequestHandler(
		(params: AddLibrarySymbolsParams) => {
		...
		EventHandlerUtils.runSafeAsync(
			async () => {
				...
				const diagnostics: LS.Diagnostic[] =
					await getDocumentDiagnostics(document, cancellationToken);
