...
export function createTraceManager(
	...
	if (SettingsUtils.getServerSettings().isBenchmarksEnabled) {
		return (
			createBenchmarkTraceManager(uri, sourceAction, position) ??
...
function createBenchmarkTraceManager(
	...
	const logDirectory: string = path.join(
		process.cwd(),
		"vscode-powerquery-logs",
	);
	...
	for (let iteration: number = 0; iteration < 1000; iteration += 1) {
		benchmarkUri = path.join(
			logDirectory,
			`${source}_${sourceAction}_${iteration}.log`,
		);
		...
		if (!fs.existsSync(benchmarkUri)) {
			...
			return new PQP.Trace.BenchmarkTraceManager((message: string) =>
