...
function throwInvalidPosition(): void {
	throw new Error(
		"The Position argument isn't in the form of lineNumberInteger,characterNumberInteger",
	);
}

function parsePosition(raw: string): PQLS.Position {
	const components: ReadonlyArray<string> = raw
		.split(":")
...
if (args.length < 4) {
	...
	throw new Error(
		`Expected the 3rd argument to be a filepath to an existing file. Received ${args[2]}`,
	);
}
...
const library: PQLS.Library.ILibrary = LibraryUtils.createLibrary(
	[
		LibrarySymbolUtils.getSymbolsForLocaleAndMode(
			PQP.Locale.en_US,
			"Power Query",
		),
	],
...
const inspectionSettings: InspectionSettings =
	PQLS.InspectionUtils.inspectionSettings(
		{
			...PQP.DefaultSettings,
			traceManager: new PQP.Trace.BenchmarkTraceManager(
