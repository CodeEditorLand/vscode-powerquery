...
function assertLibrarySymbol(symbol: unknown): asserts symbol is LibrarySymbol {
	...
	if (
		librarySymbol.documentation !== null &&
		librarySymbol.documentation !== undefined
	) {
		assertLibrarySymbolDocumentation(librarySymbol.documentation);
	}

	if (
		librarySymbol.functionParameters !== null &&
...
function assertLibrarySymbolDocumentation(
	doc: unknown,
...
function assertLibrarySymbolFunctionParameter(
	param: unknown,
	...
	const functionParam: LibrarySymbolFunctionParameter =
		param as LibrarySymbolFunctionParameter;

	if (
		functionParam.sampleValues !== null &&
		functionParam.sampleValues !== undefined
	) {
		assertIsArray(functionParam.sampleValues);
	}

	if (
		functionParam.allowedValues !== null &&
		functionParam.allowedValues !== undefined
	) {
	...
	if (
		functionParam.enumNames !== null &&
		functionParam.enumNames !== undefined
	) {
		assertIsArray(functionParam.enumNames);
	}

	if (
		functionParam.enumCaptions !== null &&
		functionParam.enumCaptions !== undefined
	) {
		assertIsArray(functionParam.enumCaptions);
	}
}

function assertLibrarySymbolRecordField(
	field: unknown,
...
function assertHasProperty(
	...
	if (!(propName in obj)) {
		...
		throw new TypeError(
			`Expected property type ${type} for property ${propName}`,
		);
	}
}
