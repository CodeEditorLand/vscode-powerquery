...
export class LibrarySymbolClient implements PowerQueryApi {
	...
	public onModuleLibraryUpdated(
		...
		if (this.lsClient.isRunning()) {
			...
			this.lsClient.error(
				"Received moduleLibraryUpdated call but client is not running.",
				undefined,
				false,
			);
		}
	...
	public async addLibrarySymbols(
		...
		if (this.lsClient.isRunning()) {
			...
			const librarySymbolsTuples: ReadonlyArray<
				[string, LibraryJson | null]
			> = Array.from(librarySymbols.entries());
			...
			this.lsClient.error(
				"Received addLibrarySymbols call but client is not running.",
				undefined,
				false,
			);
		}
	...
	public async removeLibrarySymbols(
		...
		if (this.lsClient.isRunning()) {
			...
			this.lsClient.error(
				"Received removeLibrarySymbols call but client is not running.",
				undefined,
				false,
			);
		}
