...
async function processText(
	...
	try {
					...
					async (selection: vscode.Selection) => {
						try {
							...
							await vscode.window.showErrorMessage(
								`Failed to transform text. Error: ${err}`,
							);
						}
					},
				);
		...
		await vscode.window.showErrorMessage(
			`Failed to transform text to ${target}. Error: ${err}`,
		);
	}
}
...
export async function escapeMText(
	textEditor: vscode.TextEditor,
	edit: vscode.TextEditorEdit,
): Promise<void> {
...
export async function unescapeMText(
	textEditor: vscode.TextEditor,
	edit: vscode.TextEditorEdit,
): Promise<void> {
...
export async function escapeJsonText(
	textEditor: vscode.TextEditor,
	edit: vscode.TextEditorEdit,
): Promise<void> {
...
export async function unescapeJsonText(
	textEditor: vscode.TextEditor,
	edit: vscode.TextEditorEdit,
): Promise<void> {
...
export async function extractDataflowDocument(): Promise<
...
> {
	...
	try {
		...
		await vscode.window.showErrorMessage(
			`Failed to parse document. Error: ${JSON.stringify(err)}`,
		);
	...
	if (!dataflow || !dataflow["pbi:mashup"]?.document) {
		await vscode.window.showErrorMessage(
			`Failed to parse document as a dataflow.json model`,
		);
