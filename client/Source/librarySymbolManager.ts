...
export type MinimalClientTrace = Pick<
...
export class LibrarySymbolManager {
	...
	public async refreshSymbolDirectories(
		...
		const normalizedDirectoryUris: ReadonlyArray<vscode.Uri> =
			directories.map((directory: string) => {
				...
				if (directory !== normalized) {
					this.clientTrace?.info(
						`Normalized symbol file path '${directory}' => '${normalized}'`,
					);
				}
		...
		const dedupedDirectoryUris: ReadonlyArray<vscode.Uri> = Array.from(
			new Set(normalizedDirectoryUris),
		);
		...
		const allSymbolFiles: ReadonlyArray<[vscode.Uri, LibraryJson]> = (
			await Promise.all(symbolFileActions)
		).filter(
			(value: [vscode.Uri, LibraryJson] | undefined) =>
				value !== undefined,
		) as ReadonlyArray<[vscode.Uri, LibraryJson]>;

		const validSymbolLibraries: Map<string, LibraryJson> = new Map<
			string,
			LibraryJson
		>();
		...
		this.clientTrace?.info(
			`Registering symbol files. Total file count: ${validSymbolLibraries.size}`,
		);
		...
		if (validSymbolLibraries.size > 0) {
			await this.librarySymbolClient
				...
				.then(() =>
					this.registeredSymbolModules.push(
						...validSymbolLibraries.keys(),
					),
				);
		}
	...
	public async getSymbolFilesFromDirectory(
		...
		const files: [string, vscode.FileType][] =
			await this.fs.readDirectory(directory);
		...
		return files
			.map((value: [string, vscode.FileType]): vscode.Uri | undefined => {
				...
				if (
					...
					fileName
						.toLocaleLowerCase()
						.endsWith(LibrarySymbolManager.SymbolFileExtension)
				) {
			...
			.filter(
				(value: vscode.Uri | undefined) => value !== undefined,
			) as vscode.Uri[];
	}

	public async processSymbolFile(
		fileUri: vscode.Uri,
		...
		try {
			...
			const library: LibraryJson =
				LibrarySymbolUtils.parseLibraryJson(text);

			this.clientTrace?.debug(
				`Loaded symbol file '${fileUri.toString()}'. Symbol count: ${library.length}`,
			);
	...
	private static getModuleNameFromFileUri(fileUri: vscode.Uri): string {
		return path.basename(
			fileUri.fsPath,
			LibrarySymbolManager.SymbolFileExtension,
		);
	}
