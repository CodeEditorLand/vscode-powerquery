...
suite("Should get diagnostics", async () => {
	...
	await vscode.window.showInformationMessage(
		`Starting tests using based file: ${docUri}`
	);
...
suite("Experimental diagnostics", () => {
	const docUri: vscode.Uri = TestUtils.getDocUri(
		"Diagnostics.TableIsEmpty.Error.pq"
	);
...
async function testDiagnostics(
	...
	const actualDiagnostics: vscode.Diagnostic[] =
	...
	if (expectedDiagnostics.length === 0 && actualDiagnostics.length !== 0) {
		assert.fail(
			`Expected 0 diagnostics but received: ${JSON.stringify(
				actualDiagnostics,
				undefined,
				2
			)}`
	...
	expectedDiagnostics.forEach(
		(expectedDiagnostic: vscode.Diagnostic, index: number) => {
			const actualDiagnostic: vscode.Diagnostic | undefined =
				actualDiagnostics[index];
