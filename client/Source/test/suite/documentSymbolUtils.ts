...
export async function testDocumentSymbols(
	...
	const documentSymbols: vscode.DocumentSymbol[] | undefined =
		await documentSymbolsBase(docUri);

	if (documentSymbols === undefined) {
		assert.fail("documentSymbols undefined");
	}

	const actualSymbols: ExpectedDocumentSymbol[] =
		documentSymbolArrayToExpectedSymbols(documentSymbols);
...
async function documentSymbolsBase(
	docUri: vscode.Uri
...
function documentSymbolArrayToExpectedSymbols(
	documentSymbols: vscode.DocumentSymbol[]
): ExpectedDocumentSymbol[] {
