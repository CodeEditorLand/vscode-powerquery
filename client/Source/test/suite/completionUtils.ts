...
export async function testCompletion(
	...
	const actualCompletionList: vscode.CompletionList | undefined =
		await testCompletionBase(docUri, position);
	...
	if (
		vertification === VertificationType.Exact ||
		vertification === VertificationType.Ordered
	) {
		expectedCompletionList.items.forEach(
			(expectedItem: vscode.CompletionItem, index: number) => {
				const actualItem: vscode.CompletionItem | undefined =
					actualCompletionList.items.at(index);
		...
		expectedCompletionList.items.forEach(
			(expectedItem: vscode.CompletionItem) => {
				const filteredItems: vscode.CompletionItem[] =
					actualCompletionList.items.filter(
				...
				assert.equal(
					...
					1,
					`expected to find one item with label '${expectedItem.label}'`
				);
