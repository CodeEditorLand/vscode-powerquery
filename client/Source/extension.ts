...
export async function activate(
...
): Promise<PowerQueryApi> {
	// Register commands
	context.subscriptions.push(
		vscode.commands.registerCommand(
			CommandConstant.ExtractDataflowDocument,
			CommandFn.extractDataflowDocument,
		),
		vscode.commands.registerTextEditorCommand(
			CommandConstant.EscapeJsonText,
			CommandFn.escapeJsonText,
		),
		vscode.commands.registerTextEditorCommand(
			CommandConstant.EscapeMText,
			CommandFn.escapeMText,
		),
		vscode.commands.registerTextEditorCommand(
			CommandConstant.UnescapeJsonText,
			CommandFn.unescapeJsonText,
		),
		vscode.commands.registerTextEditorCommand(
			CommandConstant.UnescapeMText,
			CommandFn.unescapeMText,
		),
	);
	...
	const debugOptions: LC.ForkOptions = {
	...
	context.subscriptions.push(
		vscode.workspace.onDidChangeConfiguration(
			async (event: vscode.ConfigurationChangeEvent) => {
				const symbolDirs: string =
					ConfigurationConstant.BasePath.concat(
						".",
...
async function configureSymbolDirectories(): Promise<void> {
	const config: vscode.WorkspaceConfiguration =
		vscode.workspace.getConfiguration(ConfigurationConstant.BasePath);
